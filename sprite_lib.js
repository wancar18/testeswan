function CSpriteLibrary() {
    ...
    this._onSpriteLoaded = function(szKey) {
        console.log('[SpriteLib] Sprite loaded:', szKey, '(' + _iCntSprites + '/' + _iNumSprites + ')');
        _cbCompleted.call(_cbOwner);
        if (++_iCntSprites === _iNumSprites) {
            console.log('[SpriteLib] ✅ All sprites loaded!');
    ...
    this.loadSprites = function() {
        ...
        for (var szKey in _oSpritesToLoad) {
            ...
            _oSpritesToLoad[szKey].oSprite.onerror = function(evt) {
                ...
                console.error('[SpriteLib] ❌ Failed to load sprite:', oSpriteToRestore.szKey, 'path:', _oSpritesToLoad[oSpriteToRestore.szKey].szPath);
                console.log('[SpriteLib] Retrying in 500ms...');
